...
class AkodoBushi(Combatant):
    ...
    def r5t_trigger(self, check, light, total):
        ...
        if self.rank == 5:
            ...
            if damage:
                self.log(
                    "spends {} vps to deal {} light wounds".format(
                        int(ceil(damage / 10)), damage
                    )
                )
    ...
    def disc_bonus(self, roll_type, needed):
        ...
        if (
            len(remaining) > 1
            and sum(remaining) >= 30
            and roll_type in ["attack", "double_attack"]
        ):
    ...
    def wc_bonus(self, light, check):
        ...
        if self.rank >= 4:
            while self.need_higher_wc(light, check):
                ...
                needed = 1
                if self.calc_serious(light, check + mb) == self.calc_serious(
                    light, check + mb + 5
                ):
                    needed = 2
